...
| [`auki-qr-detector`](crates/auki-qr-detector) | Reference `CameraDetector` package: QR Lab decode → Detection Log `type = "qr"` (`DetectorBody::Qr`) | ✓ |
| [`auki-barcode-detector`](crates/auki-barcode-detector) | First-class `CameraDetector`: rxing barcode decode → Detection Log `type = "barcode"` (`DetectorBody::Barcode`) | ✓ |
