...
Python betterproto bindings for [`auki-datatypes`](../../../crates/auki-datatypes) — dataclass-shaped types for the Auki SDK's shared cross-language data types. Lets a Python consumer encode / decode every log payload and stream wire shape that ships in the SDK without hand-rolling prost.

Filed as [Step 9 of the `auki-datatypes` migration sprint](../../../crates/auki-datatypes/src/sprint.md). Closes the typed-message half of [`detectors`](https://github.com/aukilabs/detectors) phase-2 blocker #4 — the bytes-level half [shipped in `auki-logs-py`](../auki-logs-py).
...
For contributors who edit `.proto` files in [`auki-datatypes/proto/`](../../../crates/auki-datatypes/proto/):
...
Crate landed 2026-05-09 — Step 9 of the [`auki-datatypes` migration sprint](../../../crates/auki-datatypes/src/sprint.md). Generated files committed alongside the regen workflow. **All seven on-disk Rust locked vectors plus the JointEncoders vector match byte-for-byte in Python.**
