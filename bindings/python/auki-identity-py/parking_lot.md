...
Open questions specific to the Python bindings. Cross-cutting questions about the underlying primitives belong in [`auki-identity/parking_lot.md`](../../../crates/auki-identity/parking_lot.md) or [`auki-network/parking_lot.md`](../../../crates/auki-network/parking_lot.md).

---

## PyPI distribution

Today this crate is installed from Git only — `maturin develop` against the in-tree checkout for development, or `pip install git+https://github.com/aukilabs/auki-sdk.git@<tag>#subdirectory=bindings/python/auki-identity-py` for downstream consumers (Boosterapp's sidecar). Both paths require a Rust toolchain at install time.
