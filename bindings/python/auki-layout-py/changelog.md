...
**Package relocated to `bindings/python/auki-layout-py`.** The Python package moved from `crates/auki-layout-py` to `bindings/python/auki-layout-py` with no package-name, module-name, or runtime behavior changes. Cargo workspace membership and local path dependencies now point at the new location.
