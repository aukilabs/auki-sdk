...
PyO3 bindings for [`auki-logs`](../../../crates/auki-logs)'s `Log<T>` framing primitive — opaque-bytes Python surface that lets Python producers and consumers participate in the SDK's segmented ring-buffer log on equal footing with Rust. It also creates SDK-owned retained stream sources that `auki-network-py` can serve with `StreamDecision.accept_source(source)`.
...
```sh
cd bindings/python/auki-logs-py
