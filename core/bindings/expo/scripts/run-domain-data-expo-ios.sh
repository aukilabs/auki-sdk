...
node -e '(async()=>{for(let i=0;i<900;i++){try{const p=await(await fetch("http://127.0.0.1:18114/__phase",{signal:AbortSignal.timeout(1000)})).json();if(p.phase==="failed")throw Error(JSON.stringify(p));if(p.phase==="passed"){if(p.count!==7)throw Error("incorrect case count");console.log(JSON.stringify(p));return;}}catch(e){if(String(e).includes("failed")||String(e).includes("case count"))throw e;}await new Promise(r=>setTimeout(r,100));}throw Error("Expo iOS host did not finish");})().catch(e=>{console.error(e.message);process.exit(1);});' | tee "$domain_artifacts/result.log"
