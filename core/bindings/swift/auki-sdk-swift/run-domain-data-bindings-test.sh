...
node -e 'fetch(process.argv[1]+"/__stats").then(r=>r.json()).then(s=>{if(s.refreshes!==1||s.importedRefreshes!==1||s.exchanges!==6||s.p2pExchanges!==2||s.multipartCompletions!==1||s.multipartAborts!==1||s.outstandingUploads!==0||s.records!==1||s.requests["GET /api/v1/domains"]!==2||s.requests["GET /api/v1/accessible-domains"]!==1)throw Error(JSON.stringify(s));console.log("PASS Swift FFI renewal, imported listing/data, persistence, and cleanup counters")}).catch(e=>{console.error(e);process.exit(1)})' "$base_url"
