...
export async function startFixture() {
  const state = { requests: [], denyLogin: false, denyDomains: false, denyPortals: false, denyData: false, emptyDomains: false, holdReads: false, holdLogin: false, aborted: 0, exchanges: [] };
  ...
  const releaseReads = () => { state.holdReads = false; for (const e of state.exchanges) if (e.held && e.path !== '/user/login') e.release(); };
  ...
  const wrap = handler => async (req, res) => {
    ...
    const exchange = req.fixtureExchange = { path: url.pathname, held: false, aborted: false, finished: false };
  ...
  const dataServer = http.createServer(wrap(async (req, res, url) => {
    ...
    if (url.pathname.includes(OTHER)) return send(res, 200, url.pathname.endsWith('/data') ? { data: [] } : { poses: [] });
    ...
    if (url.pathname.endsWith('/data')) {
      const matches = records.filter(record => (!url.searchParams.has('name') || url.searchParams.get('name') === record.name)
    ...
    const selected = records.find(record => url.pathname.endsWith(record.id));
    if (selected) {
  ...
  return { state, base, dataBase, content, waitFor, releaseReads, close: async () => { for (const e of state.exchanges) e.release?.(); primary.closeAllConnections(); dataServer.closeAllConnections(); await Promise.all([new Promise(resolve => primary.close(resolve)), new Promise(resolve => dataServer.close(resolve))]); } };
}
