...
| [`auki-network-py`](auki-network-py) | `ClusterRuntime` + typed `Stream<T>` + `DiscoveryClient` for Python. Sidecars stay sync-shaped; the SDK owns the asyncio loop on its tokio worker. |
