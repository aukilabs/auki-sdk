...
/// Hashes an RFC 8785 canonical representation of a manifest.
///
/// This is the same JCS + XXH3-128 content-addressing convention used by the
/// rest of the SDK. A reference therefore remains stable across serializers
/// that differ only in object-key ordering or insignificant whitespace.
pub fn manifest_hash(manifest: &impl Serialize) -> ManifestHash {
    let value = serde_json::to_value(manifest).expect("component manifests must serialize");
    let canonical = auki_jcs::canonicalize(&value);
    auki_hash::hash_jcs_bytes(&canonical)
}
...
/// Immutable identity of one exact Product Manifest.
#[derive(Clone, Debug, Eq, PartialEq, Hash, Serialize, Deserialize)]
...
/// One immutable, consistently-revisioned view of a peer's Catalog.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
...
/// A viewer-neutral Catalog projection for one Peer.
///
...
impl Catalog {
    ...
    pub(crate) fn set_current_product_input(
        ...
        let manifest_hash = manifest.hash();
        let mut state = self.inner.write().unwrap();
        if manifest.product.peer_id != manifest.peer_id {
            let expected = manifest.product.hash();
            if manifest.product_manifest_hash != expected {
        ...
        } else if let Some(product) = state.products.get(&manifest.product.product_id) {
            ...
            return Err(CatalogError::UnknownProduct(
                manifest.product.product_id.clone(),
            ));
        }
        let component = state
            .components
    ...
    /// Return the current monotonic local revision.
    pub fn revision(&self) -> u64 {
        self.inner.read().unwrap().revision
    }

    /// Capture components and products under one read lock.
...
impl<T: Send + Sync + 'static> Observable<T> {
    ...
    ///
    /// This fresh-only Observable has no retained backing. The
...
impl<I, R> Operable<I, R> {
    ...
    pub fn owner(&self) -> &ComponentReference {
        &self.owner
    }
...
///
/// Cancellation and deadlines are non-cooperative: they
...
/// An in-memory transport used for local delivery and semantic parity tests.
...
/// existing port's borrowed callback shape.
