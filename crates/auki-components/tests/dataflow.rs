...
use auki_components::{
    Buffer, BufferLimits, ConnectionOptions, CursorRead, CursorStart, Episode, EpisodeState,
    EveryFullPolicy, InputPort, OutputPort, PumpOptions, SinkFullPolicy, StaticConnection,
    StreamPump, connect, connect_buffer,
};
...
fn static_inline_delivers_in_order_without_shared_ownership() {
    ...
    {
        let mut connection = StaticConnection::new(|value: &auki_components::Envelope<u64>| {
...
fn buffer_honors_combined_hard_limits() {
    ...
    for sequence in 0..8 {
        buffer
            .append_shared(Arc::new(auki_components::Envelope::new(
                sequence,
                sequence,
                Bytes(vec![0; 3]),
            )))
