...
Steps 1 through 6 of the [migration sprint](src/sprint.md) landed 2026-05-08:
...
**The migration is complete** at Step 7 — the five on-disk and three libp2p-wire packages cover every payload type the SDK ships today; the `placeholder.proto` smoke-test (only there to validate the prost-build pipeline before any real schema landed) is gone. See [`src/readme.md`](src/readme.md) for the current state.
