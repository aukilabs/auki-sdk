...
│   └── placeholder.proto   ← validates the prost-build pipeline; removed when the first real .proto lands
├── build.rs                ← invokes prost-build over proto/
├── src/
│   ├── lib.rs              ← re-exports generated code from OUT_DIR
...
```rust
use auki_datatypes::placeholder::PipelineCheck;          // smoke test (will go away)
...
// use auki_datatypes::frame_stream::JpegFrame;
// use auki_datatypes::point_cloud_stream::PointCloudFrame;
```

Each `.proto` package gets a Rust module under `auki_datatypes::`. Generated by `prost-build` at build time; shipped as compiled-in Rust code (no runtime codegen). Final package naming convention is in [`parking_lot.md`](parking_lot.md) — the names above are illustrative.
...
Scaffolding only. The `placeholder.proto` exists to validate the `prost-build` pipeline end-to-end; no real schemas yet. See [`src/readme.md`](src/readme.md) for the current state and [`src/sprint.md`](src/sprint.md) for the migration sequence to actual messages.
