...
├── src/
│   ├── lib.rs              ← re-exports generated code from OUT_DIR; impl_log_payload! macro
...
First real schema landed at Step 1 (2026-05-08): `auki.camera` carries `PinholeCameraLogEntry` + `DynamicIntrinsics` with locked wire-bytes and hash. The `placeholder.proto` is still around — it goes away at Step 7 when another package has done the same end-to-end proof. See [`src/readme.md`](src/readme.md) for the current state and [`src/sprint.md`](src/sprint.md) for the migration sequence to the remaining messages.
