...
2. **✓ Camera streams reuse `auki.camera.PinholeCameraLogEntry`** + **`auki.point_cloud_stream` — `PointCloudFrame`** + **`auki.stream` envelope** (libp2p wire types; landed 2026-05-08; camera reuse clarified 2026-05-18).
   - Two stream-specific `.proto` packages remain: `auki.point_cloud_stream { PointCloudFrame }` and `auki.stream` (the full envelope — `StreamMessage` oneof of `StreamRequest | StreamManifest | DeclineReason | StreamEntry | EndReason`). Camera streams carry `auki.camera.PinholeCameraLogEntry` directly, so a robot log entry payload and a Park stream entry payload can be byte-identical.
   ...
   - Locked cross-language conformance vectors for `PinholeCameraLogEntry` + `PointCloudFrame` wire bytes pinned in `auki-network::stream_protocol::tests`.
