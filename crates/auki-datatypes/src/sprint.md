...
2. **✓ `auki.frame_stream` — `JpegFrame`** + **`auki.point_cloud_stream` — `PointCloudFrame`** + **`auki.stream` envelope** (libp2p wire types; landed 2026-05-08).
   - Three `.proto` packages: `auki.frame_stream { JpegFrame }`, `auki.point_cloud_stream { PointCloudFrame }`, and `auki.stream` (the full envelope — `StreamMessage` oneof of `StreamRequest | StreamManifest | DeclineReason | StreamEntry | EndReason`).
