...
use auki_dms::{client::DmsClient, types::HeartbeatRequest};
...
async fn dms_response_tokens_and_bodies_are_absent_from_logs_and_errors() {
    ...
    lease_unauthorized.assert_calls(1);
    lease_success.assert_calls(1);
    heartbeat_unauthorized.assert_calls(1);
    heartbeat_success.assert_calls(1);
    lease_error.assert_calls(1);
    heartbeat_error.assert_calls(1);
