...
Extended the browser `BrowserDomainSession` result helpers so browser-backed operations can return dynamic SDK error messages, while preserving the fail-closed `transport_unavailable` shape for operations that are still not wired.
