...
- Generated Python and Swift expose a bounded `DomainClusterManager` facade covering cluster lifecycle, manager admission, membership inspection, participant info, domain time, clock estimates, diagnostics, catalog/resource/registry providers, catalog/resource/registry fetches, and camera/detection byte streams. Swift hosts can bootstrap with explicit advertised multiaddrs or with SDK-resolved advertised listen addresses through the generated auto-advertise helper. Browser JavaScript exposes membership/election helpers, domain DTO validators, and an `AukiDomainClient` facade that composes the `auki-network` browser transport for request/response flows; there is no browser `ClusterManager` runtime.
