//! XXH3-128 content hash for RFC 8785 canonical JSON bytes.
//!
//! Pair with [`auki_jcs::canonicalize`] to obtain a deterministic 32-character
//! lowercase hex identity for any `serde_json::Value`. Used to address
//! registry entries on disk and to pin logs to the registry version they were
//! recorded against.
...
mod tests {
    ...
    fn length_is_always_32() {
        // XXH3-128 is 128 bits → exactly 32 hex chars regardless of input size.
    ...
    /// Locked vector for empty input (XXH3-128, seed 0).
    /// Matches the upstream `xxhash-rust` 0.8 output; serves as a regression
