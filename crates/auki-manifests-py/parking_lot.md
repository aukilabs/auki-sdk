...
The Rust crate's [`PoseSource`](../auki-manifests/src/lib.rs) carries `canonical_bytes()` and `hash()` for the future "graduate to a sibling registry" path. The Python wrapper doesn't expose them yet — Python consumers don't need them today, and the canonicalize-via-JCS + XXH3 dance is reproducible in pure Python if a consumer needs to.

Re-expose if a real Python consumer needs the primitives (typically when graduating `PoseSource::Slam` or similar to a registry). Track [`auki-manifests`](../auki-manifests/parking_lot.md)'s graduation discussion.
