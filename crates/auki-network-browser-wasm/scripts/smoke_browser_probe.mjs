...
const server = http.createServer(async (req, res) => {
  try {
    ...
    const filePath = path.join(
      root,
      url.pathname === "/" ? "scripts/browser_probe_smoke.html" : url.pathname,
    );
...
try {
  ...
  await page.goto(
    `http://127.0.0.1:${port}/scripts/browser_probe_smoke.html?address=${encodeURIComponent(address)}`,
  );
  ...
  if (result.protocol !== "/auki/browser-probe/0.0.1") {
    throw new Error(`bad protocol: ${result.protocol}`);
  }
  if (result.payload.join(",") !== "7,8,9") {
    throw new Error(`bad payload: ${result.payload}`);
  }
