...
if (!address) {
  throw new Error(
    "usage: node scripts/smoke_park_two_browser_acceptance.mjs <manager-webrtc-direct-multiaddr> [park-url]",
  );
}
...
try {
  ...
  const report = {
    ...
    managerPeerId,
    peers: {
      A: { selfPeerId: peerA.selfPeerId, snapshot: summarize(snapshotA) },
      B: { selfPeerId: peerB.selfPeerId, snapshot: summarize(snapshotB) },
    },
    media: {
      publishA,
      publishB,
