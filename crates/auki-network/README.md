...
- Registry protocol `/auki/registries/0.3.0`: tagged Get + tip-only `device_model` List. New peers dial 0.3; List and `device_model` require 0.3 (no v2 List). Inbound request reads are timed (`REGISTRIES_REQUEST_TIMEOUT`). Oversized List frames return `Error { "list too large" }` instead of dropping the substream.
- Blob protocol `/auki/blobs/0.1.0`: content-addressed multi-chunk transfer (prost meta + raw payload). Served from the registry app root via `set_blob_app_root`. Inbound request reads and response writes are timed (`BLOBS_REQUEST_TIMEOUT`).
