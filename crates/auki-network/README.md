...
- Modules: `swarm`, `network_runtime`, `join_protocol`, `leave_protocol`, `heartbeat_protocol`, `membership_protocol`, `info_protocol`, `resources_protocol`, `resources_v3_protocol`, `message_protocol`, `stream_protocol`, `stream_runtime`, `app_instance`, `discovery_client`
- `/auki/leave/0.0.1`: graceful leave request/Ack (empty body; peer from noise). `NetworkRuntime::send_leave_request` waits up to 2s for Ack.
...
- Discovery: `ClusterEntry.relay_multiaddrs`, `create_cluster_with_relay_multiaddrs`, `rotate_manager_with_relay_multiaddrs`; optional `DiscoveryClient::with_authorization` / `new_with_authorization` for authenticated Manager writes (`Authorization: Bearer <domain-access-jwt>`)
