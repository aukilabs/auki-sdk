...
- Registry protocol `/auki/registries/0.2.0`: legacy Get-only (untagged `{kind,id,hash}` → `{entry}`). Still accepted inbound; outbound Get falls back here when the peer lacks 0.3.
- Registry protocol `/auki/registries/0.3.0`: tagged Get + tip-only `device_model` List. New peers dial 0.3; List and `device_model` require 0.3 (no v2 List).
- Blob protocol `/auki/blobs/0.1.0`: content-addressed multi-chunk transfer (prost meta + raw payload). Served from the registry app root via `set_blob_app_root`.
