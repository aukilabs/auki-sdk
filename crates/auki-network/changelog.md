...
The stream README now calls out the closed-`StreamDispatch` stability model, and the stale JSON-binary/JPEG parking-lot item was removed because the active stream protocol is prost and the camera wrapper is gone. Tests: `cargo check -p auki-network -p auki-network-py -p auki-domain-py -p auki-datatypes`.
