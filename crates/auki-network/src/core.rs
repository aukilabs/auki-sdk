...
impl PeerIdentity {
    ...
    /// libp2p protobuf private-key encoding. This is the browser bridge:
    /// jslibp2p can import these bytes and construct the same PeerId.
    pub fn private_key_protobuf(&self) -> Vec<u8> {
        self.keypair
            .to_protobuf_encoding()
            .expect("Auki peer identities are ed25519 and protobuf-encodable")
    }
...
mod tests {
    ...
    #[test]
    fn private_key_protobuf_round_trips_to_same_peer_id() {
        let p = PeerIdentity::from_seed(&[23u8; 32]);
        let decoded = Keypair::from_protobuf_encoding(&p.private_key_protobuf()).unwrap();
        assert_eq!(decoded.public().to_peer_id(), p.peer_id());
    }
