...
43 unit tests + 3 integration tests + 1 doctest with the `swarm` feature; 35 unit tests + 3 integration tests + 1 doctest without it. Run `cargo test -p auki-network --features swarm` for the full set; `cargo test -p auki-network` for the M0 + cluster-doc + participant set.
