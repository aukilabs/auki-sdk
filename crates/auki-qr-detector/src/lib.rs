...
use auki_typed_dataflow_experiment::{
    CameraPayloadContract, Component, ComponentBuildError, ComponentSpec, ConfiguredBufferInput,
    ConfiguredObservable, ConfiguredObservableSpec, ContractType, CursorStart, Exposure, InputPort,
    ObservableContract, Observation, ObservationAccess, PayloadContract, PeerRuntime, ProductForm,
    ProductInputContract, RetainedProduct, StructuredPayloadContract, VideoFrame,
};
use futures::Stream;
...
/// source identity exposed through the typed-dataflow Catalog.
...
/// Typed-dataflow datatype emitted by [`QrDetectorComponent`].
pub const QR_DETECTIONS_DATATYPE: &str = "auki.qr-detections";

/// Typed-dataflow schema emitted by [`QrDetectorComponent`].
...
mod tests {
    ...
    fn typed_component_consumes_and_rebinds_camera_buffer_products() {
        ...
        let runtime = PeerRuntime::new("peer-a");
        let camera = runtime
            .component(
