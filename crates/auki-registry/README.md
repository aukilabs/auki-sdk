...
- `DeviceModelRegistryEntry` (`peer_id`, `device_model_id`, body) + tagged `DeviceModelFormat::Urdf { urdf_sha256, meshes }` (glTF later). `device_model_id` is the List/Get key; `body.model_id` is metadata (usually the URDF robot name) and may differ. Mesh `path` must be package-relative — `validate()` rejects `..` and absolute paths (the consumer trust boundary). `write_device_model` requires every referenced blob to already exist and updates an on-disk `TIP` pointer so List returns one tip per id.
- `put_blob` / `get_blob` / `read_blob_range` — SHA-256 content-addressed bytes under `<app_root>/blobs/<sha256>` (cap `MAX_BLOB_BYTES`).
- `put_urdf_package(app_root, urdf_path, root_convention, package_root=None, mesh_substitutions=None)` — rewrite mesh filenames, blob URDF + meshes, return a ready `DeviceModelBody` (`src/urdf.rs`, `quick-xml`). Default package dir is the URDF's parent (flattened packs). Pass `package_root` for stock ROS `pkg/urdf/` + `pkg/meshes/` trees (fail-closed under that root). Optional `mesh_substitutions` map (raw `filename=` → `{advertised_path, source_path}`) swaps visuals at publish (e.g. STL→Draco GLB) without teaching the crate manufacturer JSON. Mesh paths must be package-relative and XML-safe (no `://` leftovers after rewrite; duplicate normalized paths that resolve to different files are rejected).
- `list_device_models(app_root, peer_id)` — tip-per-id List rows (TIP preferred and never displaced by mtime; mtime fallback for pre-TIP trees). Parent directory must match `id_to_segment(device_model_id)` or the candidate is skipped. TIP/entry reads are size-capped (`MAX_DEVICE_MODEL_TIP_BYTES` / `MAX_DEVICE_MODEL_ENTRY_BYTES`); missing peer dir is empty List, other peer-dir IO is error. Candidates whose referenced blobs are missing are skipped.
...
- `validate_registry_id(id)` — rejects `>`, `@`, whitespace, and path segments that are empty / `.` / `..`. Used by `Session::register_*` in `auki-session`.
