...
- `put_urdf_package(app_root, urdf_path, root_convention, package_root=None)` — rewrite mesh filenames, blob URDF + meshes, return a ready `DeviceModelBody` (`src/urdf.rs`, `quick-xml`). Default package dir is the URDF's parent (flattened packs). Pass `package_root` for stock ROS `pkg/urdf/` + `pkg/meshes/` trees (fail-closed under that root). Mesh paths must be package-relative and XML-safe (no `://` leftovers after rewrite; duplicate normalized paths that resolve to different files are rejected).
