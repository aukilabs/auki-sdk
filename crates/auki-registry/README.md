...
- `put_urdf_package(app_root, urdf_path, root_convention, package_root=None)` — rewrite mesh filenames, blob URDF + meshes, return a ready `DeviceModelBody` (`src/urdf.rs`, `quick-xml`). Default package dir is the URDF's parent (flattened packs). Pass `package_root` for stock ROS `pkg/urdf/` + `pkg/meshes/` trees (fail-closed under that root).
