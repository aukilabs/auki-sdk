//! Strict, bounded client and validated wire types for DMS relay bookings.
//!
//! This crate owns the shared requester-side HTTP contract. Relay selection,
//! reservation recovery, and peer lifecycle remain facade responsibilities.
...
pub struct RelayAuthorizationError;
...
impl RelayAuthorizationSnapshot {
    /// Bind one sensitive bearer header to its process-local credential
    /// revision without making either value readable through the public API.
    pub fn new(header: HeaderValue, revision: u64) -> Self {
        Self { header, revision }
    }

    /// Process-local revision used to reject stale unauthorized refreshes.
    pub fn revision(&self) -> u64 {
        self.revision
    }

    /// Whether the header retains the redaction marker required by the client.
...
/// Strict requester client for the DMS relay-booking endpoints.
#[derive(Clone)]
pub struct RelayBookingClient {
    base: Url,
    http: Client,
    auth: Arc<dyn RelayAuthorizationProvider>,
}
...
pub struct RelayIdempotencyKey(String);

impl RelayIdempotencyKey {
    pub fn new(value: impl Into<String>) -> Result<Self, RelayBookingClientError> {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum RelayBookingCreateDisposition {
    Created,
    Replayed,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CreateRelayBookingResponse {
    pub disposition: RelayBookingCreateDisposition,
    pub location: String,
    pub snapshot: RelayBookingSnapshot,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum RelayBookingState {
    Active,
    Canceled,
    Expired,
    Failed,
...
pub struct RelaySlotSnapshot {
    pub slot_id: Uuid,
    pub slot_index: u8,
    pub state: RelaySlotState,
    pub assignment_id: Option<Uuid>,
    pub reservation_epoch: Option<Uuid>,
    pub provider_peer_id: Option<String>,
    pub provider_base_addresses: Option<Vec<String>>,
    pub limits: Option<RelayLimits>,
    pub provider_lease_expires_at: Option<DateTime<Utc>>,
...
pub struct RelayBookingSnapshot {
    ...
    pub provider_ready_count: u8,
    pub unfilled_count: u8,
    pub created_at: DateTime<Utc>,
    pub ended_at: Option<DateTime<Utc>>,
    pub slots: Vec<RelaySlotSnapshot>,
}
...
impl RelayErrorCode {
    ...
    pub fn is_invalid_requester_principal(self) -> bool {
        matches!(
            self,
    ...
    pub fn is_stale_requester_principal(self) -> bool {
        matches!(
            self,
...
pub enum RelayBookingClientError {
...
impl RelayBookingClientError {
    ...
    pub fn http_code(&self) -> Option<RelayErrorCode> {
        match self {
...
mod tests {
    use super::*;
    use httpmock::{Then, prelude::*};
    use serde_json::{Value, json};
    use std::sync::atomic::{AtomicUsize, Ordering};

    fn sensitive_authorization(token: &str) -> HeaderValue {
        let mut value = HeaderValue::from_str(&format!("Bearer {token}"))
            .expect("test tokens form valid header values");
    ...
    #[derive(Clone)]
    struct HeaderProvider(HeaderValue);
    ...
    #[tokio::test]
    async fn sends_the_exact_requester_booking_lifecycle_contract() {
        let server = MockServer::start();
        let token = "peer-bound-requester-machine-token";
    ...
    async fn refreshes_once_on_401_and_returns_redacted_conflict_metadata() {
        ...
        let conflict_client = make_client(
            &conflict_server,
            Arc::new(StaticProvider("requester-token-b")),
        );
    ...
    #[tokio::test]
    async fn rejects_non_sensitive_or_malformed_authorization_without_sending_or_leaking_it() {
        let server = MockServer::start();
        ...
        for (raw, sensitive) in [
            ("Bearer non-sensitive-secret", false),
            ("Basic malformed-scheme-secret", true),
            ("bearer wrong-case-secret", true),
            ("Bearer ", true),
            ("Bearer whitespace secret", true),
        ] {
            let mut authorization = HeaderValue::from_str(raw).unwrap();
            authorization.set_sensitive(sensitive);
            let error = make_client(&server, Arc::new(HeaderProvider(authorization)))
                .active()
                .await
                .unwrap_err();
            assert!(matches!(
                error,
                RelayBookingClientError::Authentication {
                    operation: RelayOperation::Active
                }
            ));
            let rendered = format!("{error:?} {error}");
            ...
            for secret in [
                "non-sensitive-secret",
                "malformed-scheme-secret",
                "wrong-case-secret",
                "whitespace secret",
            ] {
        ...
        unexpected_request.assert_calls(0);
    }

    #[tokio::test]
    async fn stops_after_a_failed_401_refresh_without_leaking_credentials_or_response_body() {
        const SECRET_BODY: &str = "unauthorized-response-secret";
        let server = MockServer::start();
        let unauthorized = server.mock(|when, then| {
            when.method(GET)
                .path("/relay-bookings/active")
                .header("authorization", "Bearer requester-refresh-secret");
            relay_json(
                then,
                401,
                json!({"code": "unauthorized", "error": SECRET_BODY}),
        ...
        unauthorized.assert_calls(1);
        let rendered = format!("{error:?} {error}");
        assert!(!rendered.contains("requester-refresh-secret"));
    ...
    async fn preserves_the_configured_api_prefix_and_validates_prefixed_locations() {
        ...
        let client =
            RelayBookingClient::new(base, Arc::new(StaticProvider("requester-token"))).unwrap();
