...
Lean: (1). Aligning Rust submodules to Python namespacing is the cheaper convergence — Rust gets clearer organization at no cost while still pre-1.0; Python keeps its current shape; the [`auki-session`/`auki-paths` rename question](auki-session/parking_lot.md) becomes more obvious because `auki_session::session::Session` would naturally pair with `auki_session_py.session.Session`. Apply the same convention to future `*-py` crates as they land.
...
- [`auki-logs/`](auki-logs/parking_lot.md) — per-entry checksums; reader streaming for unbounded captures; encoder-aware vs encoder-agnostic `Log<T>` post-migration (decided in step 1 of the auki-datatypes migration)
