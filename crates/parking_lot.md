...
- [`auki-registry/`](auki-registry/parking_lot.md) — UTC clock epoch format; sensor_id naming convention formalization; atomic-write tmp cleanup (Frame Registry shape RESOLVED 2026-05-07 — `FrameRegistryEntry { handedness, axes, units }` + four preset constructors landed in v0.0.22, no `parent_frame` on the entry, no `label`)
