...
1. **Mirror Python's namespacing in Rust.** Introduce real submodules at the Rust crate root: `auki_network::cluster::{ParticipantInfo, ClusterRuntime, ClusterDoc, …}`, `auki_network::discovery::DiscoveryClient`, possibly `auki_network::stream::{StreamProvider, StreamDispatch, PinholeCameraLogEntry, PointCloudFrame, …}`. Drop the flat `pub use` re-exports (or keep them with `#[deprecated]` for one release). Pre-1.0; non-breaking is cheap right now.
...
Python binding package summaries moved to [`../bindings/python/parking_lot.md`](../bindings/python/parking_lot.md).
...
- [`auki-network-swift/`](auki-network-swift/parking_lot.md) — **new crate, landed 2026-05-19** (Stage 1 UniFFI Discovery binding for `aukilabs/iosapp`); async-shaped Swift API vs the `-py` sync precedent (**flagged for human confirmation**); where generated Swift / XCFramework artifacts live + committed-vs-built distribution; iOS crypto backend `ring` vs `aws-lc-rs`; stream-payload parity rule for Stage 2
