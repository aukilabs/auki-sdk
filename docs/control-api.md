...
| `session_now_ns` | integer | The session clock's current value at the moment this response was generated. Strictly increasing across responses. Consumers wanting wall-clock time subtract at poll time (e.g. `consumer_utc_now − session_now_ns ≈ session_started_at_consumer_utc`); the principled cross-clock path is `convert_time` via the [TimeTransform Log](../crates/auki-time/README.md). |
