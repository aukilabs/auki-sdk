...
| 4 | Component identity is stable behavior identity; Output identity names one immutable configured production contract. | **Revise** | Terminal reconfiguration removes the need for subscriptions to survive through a stable Output slot. Products still need to pin the contract that produced them, but it is unresolved whether that requires a separate Output identity or can reference a configured Component Manifest. |
...
| 6 | Component Manifests, Output Manifests, and Product Manifests are distinct fetchable Catalog descriptions. | **Revise** | Component and Product Manifests remain justified. A separate Output Manifest is now provisional and must earn its existence independently of automatic subscription migration. The current JSON hashing is also not a canonical format. |
...
- whether configured Outputs require identities and Manifests separate from
  configured Component Manifests;
- whether named output slots are useful beyond Components with multiple real
...
4 Component / configured Output identity:    Accept | Revise | Reject
5 Typed compatibility, not hash equality:    Accept | Revise | Reject
6 Separate Output Manifest:                  Accept | Revise | Reject
...
14 Terminal reconfiguration notice:          Accept | Revise | Reject
