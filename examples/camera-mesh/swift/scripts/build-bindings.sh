...
for TARGET in aarch64-apple-ios aarch64-apple-ios-sim x86_64-apple-ios; do
  cargo build --locked --release -p auki-camera-mesh-swift --target "$TARGET"
done
...
cargo run --locked --release --features cli \
  -p auki-camera-mesh-swift --bin uniffi-bindgen -- generate \
  --library "$DEVICE_LIB" \
  --language swift \
  --out-dir "$BINDINGS"
...
xcodebuild -create-xcframework \
  ...
  -output "$ARTIFACTS/AukiCameraMesh.xcframework"
...
swiftc \
  ...
  "$SWIFT_OUT"/*.swift
plutil -lint "$ARTIFACTS/AukiCameraMesh.xcframework/Info.plist"

echo "XCFramework: $ARTIFACTS/AukiCameraMesh.xcframework"
