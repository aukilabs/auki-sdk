...
get<HTMLFormElement>("send").onsubmit = (event) => (event.preventDefault(), void sendEcho());
stopButton.onclick = () => void stopPeer();
async function startPeer(): Promise<void> {
  ...
  try {
    ...
    void started.waitStopped().catch((error) => peer === started && (write(error), void stopPeer()));
  } catch (error) {
...
async function stopPeer(): Promise<void> {
  ...
  if (pendingReceive) await pendingReceive;
  mounted.free();
  running.free();
...
function write(value: unknown): void {
  log.textContent += `${value instanceof Error ? value.message : String(value)}\n`;
}
