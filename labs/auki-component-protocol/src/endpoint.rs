...
use auki_components::{
    ...
    Invocation, InvocationContext, InvocationError, Observation, ObservationEnd, Operable,
...
impl ComponentProtocolEndpoint {
        ...
        T: Serialize + Send + Sync + 'static,
    {
        ...
        let handler: Arc<ProductHandler> = Arc::new(move |selection| {
            // Read terminal state first. Once present, this Buffer is closed
            // and its retained tail cannot move while we select a batch.
            let terminal = exported.end_notice();
            let from_sequence = match &selection {
                ObservationSelection::FromSequence { sequence, .. } => Some(*sequence),
                _ => None,
            };
            let (observations, gap) = select_observations(&exported, selection)?;
            let end = terminal.filter(|_| {
                from_sequence.is_none_or(|requested| {
                    exported.buffer().range().last_sequence.is_none_or(|tail| {
                        requested > tail
                            || observations
                                .last()
                                .is_some_and(|item| item.sequence >= tail)
                    })
                })
            });
...
pub struct RemoteObservations<T> {
    ...
    /// Source termination. Sequence reads reveal it only at the retained tail.
    pub end: Option<ObservationEnd>,
}
    ...
    T: DeserializeOwned + Send + Sync + 'static,
{
    ...
    /// Inspect why this exact source ended. A replacement is only a hint;
    /// attaching to it requires constructing a new mirror explicitly.
    ...
    pub fn rebind_exact_route(
        ...
        let route = match &mut self.route {
            RemoteRoute::Exact { route, .. } => route,
            #[cfg(not(target_arch = "wasm32"))]
            RemoteRoute::Configured(_) => {
    ...
    pub async fn sync_once(&mut self) -> Result<RemoteProductSync, ComponentProtocolError> {
        if let Some(ended) = self.terminal_sync() {
            return Ok(ended);
        }
        let batch = self
    ...
    async fn exchange(
        ...
        let batch = match observations_exchange(&mut stream, request).await {
            ...
            Err(error) => {
                // Discard a failed transport so an explicit retry reconnects
                // with the last successfully retained source cursor.
                return Err(error);
            }
        };
        Ok(batch)
    ...
    fn finish_sync(
        ...
        let accepted = self.ingest(batch.observations, gap)?;
        // Do not close the mirror if an append failed: that batch and its end
        // notice must remain retryable from the last successfully stored value.
        self.accept_end(batch.end.clone())?;
    ...
    fn terminal_sync(&self) -> Option<RemoteProductSync> {
        self.end_notice().map(|end| RemoteProductSync {
            accepted: 0,
            gap: None,
            next_sequence: self.next_sequence,
            end: Some(end),
    ...
    fn accept_end(&self, end: Option<ObservationEnd>) -> Result<(), ComponentProtocolError> {
        if let Some(end) = end {
            self.product
                .close_with_notice(end)
                .map_err(|error| ComponentProtocolError::Import(error.to_string()))?;
            self.observation_stream
                .lock()
                .expect("observation stream lock poisoned")
    ...
    fn ingest(
        ...
        gap: Option<SourceGap>,
    ) -> Result<usize, ComponentProtocolError> {
        self.last_gap = gap;
        let mut accepted = 0;
        for observation in observations {
            ...
            let next_sequence = observation.sequence.saturating_add(1);
            self.product
                .buffer()
...
pub struct RemoteProductSync {
    ...
    /// Present after the final selected data was retained and readers closed.
    pub end: Option<ObservationEnd>,
}
    ...
    T: DeserializeOwned,
{
    ...
    deadline(
        ...
        async {
            ...
            if end
                .as_ref()
                .is_some_and(|notice| notice.output != producer_reference)
            {
                return Err(ComponentProtocolError::InvalidResponse(
                    "terminal notice does not match the Product producer".to_owned(),
                ));
            }
