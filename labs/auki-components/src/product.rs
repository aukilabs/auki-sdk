...
pub struct RetainedProduct<T> {
    ...
    pub(crate) end: Arc<Mutex<Option<ObservationEnd>>>,
}
...
impl<T> RetainedProduct<T> {
    ...
    /// Terminal source notice, shared by all handles without changing the
    /// immutable Product manifest. Retained observations remain readable.
    pub fn end_notice(&self) -> Option<ObservationEnd> {
        self.end.lock().unwrap().clone()
    }

    /// End a local capture or imported mirror after its final data is retained.
    /// Readers may still drain that history. Repeating the same notice is
    /// harmless; replacing it is not.
    pub fn close_with_notice(&self, notice: ObservationEnd) -> Result<(), ProductImportError> {
        if notice.output != self.manifest.producer {
            return Err(ProductImportError::ProducerMismatch);
        }
        let mut end = self.end.lock().unwrap();
        if let Some(previous) = end.as_ref() {
            return if previous == &notice {
                Ok(())
            } else {
        ...
        if self
            ...
            .range()
            .last_sequence
            .is_some_and(|last| notice.last_sequence.is_none_or(|terminal| last > terminal))
        {
            return Err(ProductImportError::EndBeforeRetainedData);
        }
        *end = Some(notice);
        self.buffer.close();
        Ok(())
...
impl fmt::Display for ProductImportError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ConflictingEnd => {
                formatter.write_str("Product already has a different terminal notice")
            }
            Self::EndBeforeRetainedData => {
                formatter.write_str("terminal notice precedes retained observations")
            }
...
impl<T: Send + Sync + 'static> BufferProductCapture<T> {
    ...
    pub fn end_notice(&self) -> Option<ObservationEnd> {
        self.state.lock().unwrap().product.end_notice()
    }
        ...
        let input = observation_input(format!("{product_id}.capture"), move |event| {
            ...
            match event {
                ...
                ObservationEvent::Ended(end) => {
                    if let Err(error) = state.product.close_with_notice(end.clone()) {
                        state.errors.push(error.to_string());
                    }
                }
            }
