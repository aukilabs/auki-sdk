...
use auki_components::{
    ...
    ComponentBuildError, ComponentRuntime, ComponentSpec, ConfiguredObservableSpec, ContractType,
    CursorStart, EveryFullPolicy, Exposure, GaugePayloadContract, InMemoryTransport, InputPort,
    ...
    ObservationEvent, OperableContract, PayloadContract, ProductCaptureError, ProductForm,
    ProductInputContract, ProductState, PublishError, SerializedInMemoryTransport, SharedScheduler,
    observation_input,
};
...
#[test]
fn retained_product_end_is_shared_validated_and_does_not_change_its_manifest() {
    use auki_components::{CursorRead, ObservationEnd, ProductImportError};
    ...
    let capture = runtime
        .capture_buffer("history", &output, BufferLimits::entries(8), |_| 8)
        .unwrap();
    let product = capture.product();
    let clone = product.clone();
    let original_reference = product.reference();
    output.publish(10, Arc::new(12.5)).unwrap();
    let mut reader = product.buffer().subscribe(CursorStart::Latest);
    let end = ObservationEnd {
        output: output.reference().clone(),
        last_sequence: Some(0),
        timestamp_ns: 20,
        reason: ObservationEndReason::Failed {
            reason: "fixture source stopped".to_owned(),
        },
    };
    let mut wrong_source = end.clone();
    wrong_source.output.output_id = "other-output".to_owned();
    assert!(matches!(
        product.close_with_notice(wrong_source),
        Err(ProductImportError::ProducerMismatch)
    ));
    let mut early_end = end.clone();
    early_end.last_sequence = None;
    assert!(matches!(
        product.close_with_notice(early_end),
        Err(ProductImportError::EndBeforeRetainedData)
    ));
    assert_eq!(product.end_notice(), None);
    assert!(matches!(
    ...
    assert_eq!(
        output.end(end.timestamp_ns, end.reason.clone()).unwrap(),
        end
    );
    assert_eq!(clone.end_notice(), Some(end.clone()));
    assert_eq!(capture.end_notice(), Some(end.clone()));
    assert!(capture.errors().is_empty());
    assert_eq!(product.reference(), original_reference);
    assert_eq!(product.manifest.hash(), original_reference.manifest_hash);
    assert_eq!(*clone.latest_existing().unwrap().unwrap().payload, 12.5);
    assert!(matches!(
        reader.next_timeout(Duration::ZERO),
        CursorRead::Closed
    ));
    product.close_with_notice(end.clone()).unwrap();
    let mut conflicting = end.clone();
    conflicting.timestamp_ns += 1;
    assert!(matches!(
        product.close_with_notice(conflicting),
        Err(ProductImportError::ConflictingEnd)
    ));
    assert_eq!(product.end_notice(), Some(end));
}
