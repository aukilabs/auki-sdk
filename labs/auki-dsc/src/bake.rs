...
pub(crate) fn bake_navmesh(
    ...
    let aabb = trimesh.compute_aabb().ok_or(DscError::EmptyMesh)?;
    ...
    .build()
    .map_err(|e| DscError::Bake(e.to_string()))?;
    ...
    if poly_mesh.polygon_count() == 0 {
        return Err(DscError::EmptyNavMesh);
    }

    let nav = poly_mesh_to_landmass(&poly_mesh)?;
    let validated = nav
        .validate()
        .map_err(|e| DscError::Bake(format!("landmass validate: {e:?}")))?;
...
fn poly_mesh_to_landmass(poly: &PolygonNavmesh) -> Result<NavigationMesh<XYZ>, DscError> {
