...
/// Baked navigation mesh. Query ops take/return Y-up metres.
pub struct NavMesh {
...
impl NavMesh {
    pub fn bake(mesh: &TriangleMesh, profile: &BakeProfile) -> Result<Self, DscError> {
        let BakedNav {
            archipelago_mesh,
            agent_radius,
    ...
    fn sample(
        ...
        p: Vec3,
        extents: Extents,
    ) -> Result<SampledPoint<'_, XYZ>, DscError> {
        self.archipelago
            .sample_point(yup_to_landmass(p), &Self::sample_distance(extents))
            .map_err(|_| DscError::OffMesh)
    }
    ...
    pub fn find_path(&self, waypoints: &[Vec3]) -> Result<PathResult, DscError> {
    ...
    pub fn find_optimized_path(
        ...
        waypoints: &[Vec3],
        fixed_end: bool,
    ) -> Result<OptimizedPathResult, DscError> {
        require_waypoints(waypoints)?;
        let extents = Extents::default();
        let on_mesh: Vec<OnMeshPoint> = waypoints
            .iter()
