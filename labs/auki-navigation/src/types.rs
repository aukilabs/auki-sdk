...
impl From<auki_geometry::mesh::Vec3> for Vec3 {
    fn from(v: auki_geometry::mesh::Vec3) -> Self {
...
pub(crate) fn require_waypoints(waypoints: &[Vec3]) -> Result<(), NavError> {
    if waypoints.len() < 2 {
        Err(NavError::TooFewWaypoints)
    } else {
        Ok(())
    }
}
