...
- [**SDK-Q3**](crates/auki-domain/parking_lot.md#successor-token-encoding-for-v2-manager-handoff-hardening-filed-by-nilss-claude-2026-05-13) — Successor-token encoding (prost via `auki-proto` / JWT / bare signed JSON). Lean: prost, ~60%. **Not gating implementation** — v1 Discovery contract skips signature verification entirely, so this defers to v2.
- [**Stale-Manager join policy**](crates/auki-domain/parking_lot.md#hagall-stale-manager-join-policy--what-if-discovery-points-at-a-dead-manager-before-the-join-response-_filed-by-nilss-codex-2026-05-17_) — What should `join_cluster` do when Discovery points at a dead Manager before any membership snapshot is available? Lean: no unilateral takeover without explicit singleton semantics or a Discovery-served membership snapshot.
...
These six items came out of an API-surface review walkthrough on 2026-05-08, after PR #55 landed the first payload-migration prep. The marquee elevation finding (`TransformSample` is wrong-layered as an `auki-registry` payload helper when it's the SDK's flagship transform primitive) is **resolved** — the payload migration moved it to generated `auki-proto` bindings and renamed it `SpatialTransform` per the Pose Log synthesis decided 2026-05-07. The remaining six items are smaller; each lives in the most-specific parking-lot it belongs in. Listed here for visibility and so a future reader can find the whole set.
...
|---|---|---|
| 213 | *"v0.0.22 ships `T = PinholeCameraLogEntry` (**grimsby v1** — byte-identical to …) and `T = PointCloudFrame` (**Dagaz Batch 1** — …)"* | *"v0.0.22 ships `T = PinholeCameraLogEntry` (RGB camera streaming — byte-identical to the camera log payload) and `T = PointCloudFrame` (CDR-encoded `PointCloud2` streaming — …)"* |
...
- [`bindings/`](bindings/parking_lot.md) — Python package distribution/stubs, JavaScript/Web package target policy, Swift package distribution, and per-package binding questions
- [`crates/auki-proto/`](crates/auki-proto/parking_lot.md) — no open questions yet; active Rust protobuf payload bindings are generated from root [`proto/auki`](proto/auki), and non-Rust outputs are generated locally under ignored `bindings/*/auki-proto/` directories.
...
- [`crates/auki-session-py/`](crates/auki-session-py/parking_lot.md) — `payload: bytes` encoding contract resolved (protobuf via `auki-proto`); libp2p control-plane design timing (deferred until this crate stabilizes); 6 resolved design decisions waiting to propagate when first implementation lands
