...
- **`/auki/stream/0.1.0` substream-accept doesn't check cluster membership.** Filed 2026-05-12 at [`crates/auki-network/parking_lot.md`](crates/auki-network/parking_lot.md). `handle_inbound_substream` accepts any libp2p-routable peer's stream-open; combined with Park's `enable_mdns: true` it lets a non-cluster peer pull live frames over QUIC. Lean: surface requesting `peer_id` to the producer's `stream_provider` callback so the app decides accept-vs-decline per peer (alternative: server-side hard-locked check). Cross-references the [boosterapp visibility gap](https://github.com/aukilabs/boosterapp/blob/develop/scripts/parking_lot.md) — `/api/cluster` doesn't show stream subscribers either, so the operator can't even see the bypass.
