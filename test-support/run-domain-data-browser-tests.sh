...
pw snapshot
pw run-code 'async page => { await page.waitForFunction(() => document.body.textContent.includes("test result:"), null, {timeout:45000}); const result = await page.locator("body").innerText(); if (!result.includes("test result: ok. 15 passed; 0 failed; 0 ignored;")) throw Error(result); return result; }'
pw snapshot
